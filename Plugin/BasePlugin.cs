    ...
    public abstract class BasePlugin : IBasePlugin
    {
        ...
        public virtual void Execute(IServiceProvider serviceProvider)
        {
            ...
            try
            {
                ...
                {
                    context.Trace($"No Registered Event found for event: {context.MessageName}, Entity: {context.PrimaryEntityName}, Mode: {mode.ToString()}, and Stage: {context.PipelineStage}!");
                    return;
                }
